 ...
 * resolve 함수가 리턴 되면 state가 fulfilled(성공)으로 변경되고 'value'가 resolve 함수에 전달된다.
...
prm.then((result) => {
		...
		return result + 10;     // 새로운 Promise 객체 반환으로 2번째 then 메서드를 호출
	})
	.then((result) => {
...
그리고 아래 코드와 같이 retrun에 선언해둔 Promise를 호출하여 반환할 수도 있다.
